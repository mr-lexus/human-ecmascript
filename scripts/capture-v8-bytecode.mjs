...
function captureBytecode(source, sourcePath, { id, functionName }) {
  ...
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    ...
    if (result.status !== 0)
      throw new Error(`${functionName} capture exited with ${result.status}`);
    ...
    if (headerStart >= 0 && tableEnd >= 0) {
      capture = output.slice(headerStart, tableEnd);
      break;
    }
  }
  if (!capture) throw new Error(`Cannot isolate ${functionName} bytecode after 3 attempts`);
